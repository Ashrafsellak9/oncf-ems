#!/usr/bin/env python3
"""
Script simplifié pour importer les gares sans géométries complexes
"""
...
def import_gares_simple():
    """Importer les gares de manière simplifiée"""
    try:
        conn = psycopg2.connect(os.getenv('DATABASE_URL'))
        cursor = conn.cursor()
        
        print("🚂 Import simplifié des gares")
        print("=" * 60)
        ...
        # 3. Créer la nouvelle table simplifiée
        ...
        # 4. Importer les données avec gestion d'erreurs
        print("\n📥 Import des données:")
        success_count = 0
        error_count = 0
        ...
        for i, line in enumerate(lines, 1):
            if i % 20 == 0:
                print(f"   Progression: {i}/{len(lines)} gares (✅ {success_count}, ❌ {error_count})")
            ...
            try:
                ...
                if len(parts) >= 17:
                    ...
                    """, (
                        ...
                        parts[4] if len(parts) > 4 else None,  # geometrie (TEXT)
                        parts[5] if len(parts) > 5 else None,  # geometrie_dec (TEXT)
        ...
        print(f"   - Total: {total_gares:,} gares importées avec succès")
